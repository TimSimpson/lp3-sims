...
template <typename T> class EventSubscriberT : public EventSubscriber {
  public:
    EventSubscriberT(std::function<void(const T & args)> func)
        : EventSubscriber(), function(func) {}
...
class EventDispatcher {
  public:
    template <typename T> void send(EventType id, T args) {
    ...
    EventSubscribers &
    find_or_create_subscribers(const EventType id,
                               const std::type_index & type_index);
