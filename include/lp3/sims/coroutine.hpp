...
// =================
//
// lp3::sims coroutines are stateless coroutines which don't persist the
// values of local variables between invocations (like Lua or Boost
// coroutines) but are much cheaper are portable. They work by using a macro
// to create a simple state machine with switch statements. The result is
// nicer than the you might expect.
//
...
//  - Use independent CoroutineState variables for each coroutine. Don't
//  muck
...
// ---------------------------------------------------------------------------
class LP3_SIMS_API CoroutineState {
  public:
    // This is declared public, but only because there was no other (simple)
    // choice. It's best to leave this alone.
    int code_pointer;

    CoroutineState() : code_pointer(0) {}

    // Returns true if the coroutine is still running.
    explicit inline operator bool() const { return code_pointer != -1; }

    // Go to a place marked with
    inline void go_to(int label) { code_pointer = label * -1 - 1; }
};

class LP3_SIMS_API CoroutineFinishedException : public std::logic_error {
  public:
    CoroutineFinishedException() noexcept
        : logic_error("Coroutine was called again after completing.") {}
};

class LP3_SIMS_API CoroutineInvalidStateException : public std::logic_error {
  public:
    CoroutineInvalidStateException() noexcept
        : logic_error("The Coroutine is in a corrupted or invalid state.") {}
};
...
#define LP3_COROUTINE_END()                                                    \
    ...
    break;                                                                     \
    case (-1):                                                                 \
        throw ::lp3::sims::CoroutineFinishedException{};                       \
        break;                                                                 \
    default:                                                                   \
        throw ::lp3::sims::CoroutineInvalidStateException{};                   \
        break;                                                                 \
        }
