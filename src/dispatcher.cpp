...
namespace sims {
    ...
    EventSubscriber::EventSubscriber() : cancelled(false), id(++next_id) {}

    EventSubscribers::EventSubscribers() : need_prune(false) {}
    ...
    void EventSubscribers::cancel_subscriber(SubscriptionFuncId sf_id) {
        for (EventSubscriber * s : this->subscribers) {
            LP3_SIMS_ASSERT2(nullptr != s, "null subscriber found in list");
            if (s->id == sf_id) {
                s->cancelled = true;
                need_prune = true;
            }
    ...
    void EventDispatcher::unsubscribe(SubscriptionId id) {
        ...
        if (itr == subscriber_map.end()) {
            LP3_SIMS_ASSERT2(false, "SubscriptionId not found");
            return;
        } else {
