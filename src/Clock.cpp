...
namespace sims {
    ...
    LP3_SIMS_API
    Timer::Timer() : current_tick_count(0), init_high_freq_time(now_in_ms()) {}
    ...
    std::int64_t FrameTimer::next_frame() {
        ...
        if (max_frame_diff
            && dif > max_frame_diff.value()) { // Ensure slow down happens
                                               // instead of goofy errors.
    ...
    std::int64_t GameClock::start_frame() {
        ...
        // may have to simulate an unseemly amount of time. So we cap the
        // elapsed time we will be simulating at around half a second - meaning
        // that if the real loop can't go at least that fast, the game will
        // noticably slow down.  However it beats the alternative, which would
        // make the app increasingly unresponsive.
        if (lag > max_lag) {
            lag = max_lag;
        }
