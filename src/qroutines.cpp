...
namespace sims {
    ...
    QRoutine::QRoutine(QRoutineDelegate f, SleepTime _sleep_time)
        : func(f), sleep_time(_sleep_time), id(++next_id) {}
    ...
    bool equiv(const QRoutine & a, const QRoutine & b) { return a.id == b.id; }

    bool comp(const QRoutine * a, const QRoutine * b) {
        LP3_SIMS_ASSERT(nullptr != a);
        LP3_SIMS_ASSERT(nullptr != b);
        return comp(*a, *b);
    }
    ...
    QRoutineRunner::QRoutineRunner() : current_time_processed(0), routines() {}

    void QRoutineRunner::cancel(const QRoutineId id) {
        auto itr = std::find_if(routines.begin(), routines.end(),
                                [id](QRoutine & q) { return q.id == id; });
        if (itr != routines.end()) {
            routines.erase(itr);
        }
    }
    ...
    bool QRoutineRunner::process_events(const SleepTime time) {
        ...
        // routine B sleeps for 35. If we did not adjust the sleep time of
        // routine A it would finish completely before routine B had a chance to
        // start, as 10 > 35. However with the addition A's sleep time after
        // it's first iteration will be 40, not 10.

        LP3_SIMS_ASSERT(list_is_ordered());
        ...
        // Now iterate through and reduce the amount of sleep time from
        // everything by however much this loop was supposed to process.
